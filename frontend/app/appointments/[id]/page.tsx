    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (!aptData) {
                    setError(t("appointmentDetail.notFound"));
                } else {
                ...
                console.error(err);
                setError(t("appointmentDetail.failedToLoad"));
            } finally {
                setLoading(false);
            }
        };
        ...
        return (
            <MainLayout>
                <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
                    ...
                    <p className="text-xl font-bold text-gray-900">{error || t("appointmentDetail.notFound")}</p>
                    <Button onClick={() => router.back()}>{t("appointmentDetail.goBack")}</Button>
                </div>
            </MainLayout>
        );
    }
                ...
                <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold text-gray-900">{t("appointmentDetail.title")}</h1>
                                <p className="text-sm text-gray-500 uppercase tracking-widest font-bold mt-1">
                                    {fromHistory ? t("appointmentDetail.auditReviewMode") : t("appointmentDetail.managementView")}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <Button variant="outline" onClick={() => router.back()}>{t("common.cancel")}</Button>
                            {isAdminView && <Button variant="primary">{t("appointmentDetail.editAppointment")}</Button>}
                        </div>
                    </div>
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            <Card className="p-8 border-none shadow-2xl shadow-gray-200/50 bg-white rounded-[2.5rem]">
                                <div className="flex items-center justify-between mb-8 pb-6 border-b border-gray-100">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest">{t("appointmentDetail.status")}</p>
                                            <h2 className="text-2xl font-black text-gray-900">{appointment.status}</h2>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest text-right">{t("appointmentDetail.reference")}</p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-6">
                                        <div>
                                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{t("appointmentDetail.schedule")}</p>
                                        ...
                                        <div>
                                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">{t("appointmentDetail.clientInformation")}</p>
                                            <div className="flex items-center gap-3 text-gray-900">
                                                <User className="w-5 h-5 text-[var(--color-primary)]" />
                                                <span className="text-lg font-bold">{appointment.clientName}</span>
                                            </div>
                                            <p className="text-sm text-gray-500 mt-1 ml-8">{t("appointmentDetail.regularClient")} • {t("appointmentDetail.salonMember")}</p>
                                        </div>
                                    </div>

                                    <div className="space-y-6">
                                        <div>
                                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-3">{t("appointmentDetail.serviceDetails")}</p>
                                            <div className="space-y-3">
                                                {appointment.services && appointment.services.map((service: any) => (
                                                    <div key={service.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                                        ...
                                                        <span className="font-black text-[var(--color-primary)]">{formatCurrency(service.price)}</span>
                                                    </div>
                                                ))}
                                                {(!appointment.services || appointment.services.length === 0) && (
                                                    <p className="text-gray-500 italic">{t("appointmentDetail.noServicesListed")}</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <Card className="p-8 border-none shadow-xl shadow-gray-200/50 bg-white rounded-[2.5rem] space-y-6">
                                <h3 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                                    ...
                                    {t("appointmentDetail.internalComments")}
                                </h3>
                                <div className="space-y-4">
                                    ...
                                    {(!comments || comments.length === 0) && (
                                        <p className="text-gray-500 italic">{t("appointmentDetail.noCommentsYet")}</p>
                                    )}
                                </div>
                            </Card>
                        </div>
                        ...
                        <div className="space-y-8">
                            <Card className="p-6 border-none shadow-xl shadow-gray-200/50 bg-white rounded-[2rem] space-y-6">
                                <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
                                    ...
                                    <h3 className="font-bold text-gray-900">{t("appointmentDetail.auditTrail")}</h3>
                                </div>
                            ...
                            {income && (
                                <Card className="p-6 border-none shadow-xl shadow-blue-500/10 bg-gradient-to-br from-blue-600 to-blue-700 text-white rounded-[2rem] space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="font-bold">{t("appointmentDetail.billingAvailable")}</h3>
                                    </div>
                                    <p className="text-sm text-white/80">{t("appointmentDetail.billingDesc")}</p>
                                    <Button variant="outline" className="w-full bg-white/10 border-white/20 text-white hover:bg-white/20 py-4 rounded-2xl border-none">
                                        <Download className="w-4 h-4 mr-2" /> {t("appointmentDetail.downloadPdf")}
                                    </Button>
                                </Card>
                            )}
                            ...
                            <div className="px-4">
                                ...
                                <p className="text-[10px] text-gray-400 text-center mt-3 italic">
                                    ...
                                    {t("appointmentDetail.changesTracked")}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    {fromHistory && (
                        <div className="flex justify-center pt-8">
                            ...
                            >
                                ...
                                {t("appointmentDetail.returnToAuditLog")}
                            </Button>
                        </div>
                    )}
                </div>
