...
const approvedItems = [
    { id: 1, type: "Revenue", date: "2026-01-10", description: "Box Braids - Thomas Petit", amount: 120, approvedBy: "Admin", approvedDate: "2026-01-11" },
    ...
    { id: 3, type: "Revenue", date: "2026-01-09", description: "Goddess Braids - Isabelle Moreau", amount: 130, approvedBy: "Admin", approvedDate: "2026-01-10" },
];

export default function ValidationPage() {
    const [selectedTab, setSelectedTab] = useState<"revenues" | "expenses" | "history">("revenues");
    const [selectedItems, setSelectedItems] = useState<number[]>([]);

    const handleApprove = (id: number, type: "revenue" | "expense") => {
        console.log(`Approving ${type} #${id}`);
        // TODO: API call to approve item
    };

    const handleReject = (id: number, type: "revenue" | "expense") => {
        ...
        <MainLayout>
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Review and approve pending revenues and expenses</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card gradient="bg-gradient-to-br from-purple-600 to-purple-700" className="text-white">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm opacity-90 mb-1">Pending Revenues</p>
                                <h3 className="text-4xl font-bold">{pendingRevenues.length}</h3>
                                <p className="text-sm opacity-80 mt-1">
                                    Total: €{pendingRevenues.reduce((sum, r) => sum + r.amount, 0)}
                                </p>
                            </div>
                    ...
                    <Card gradient="bg-gradient-to-br from-orange-500 to-orange-600" className="text-white">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <h3 className="text-4xl font-bold">{pendingRevenues.length + pendingExpenses.length}</h3>
                ...
                <div className="flex gap-4 border-b border-gray-200">
                    <button
                        ...
                        className={`px-4 py-2 font-medium transition-colors ${selectedTab === "revenues"
                                ? "border-b-2 border-purple-600 text-purple-600"
                                : "text-gray-500 hover:text-gray-700"
                            }`}
                    >
                        Pending Revenues ({pendingRevenues.length})
                    </button>
                    <button
                ...
                {/* Pending Revenues Table */}
                {selectedTab === "revenues" && (
                    <Card>
                        <h3 className="text-lg font-semibold mb-4">Pending Revenues</h3>
                                        ...
                                        <tr key={revenue.id} className="hover:bg-gray-50 transition">
                                            <td className="px-4 py-4">
                                                <input
                                                    ...
                                                    onChange={() => toggleItemSelection(revenue.id)}
                                                />
                                            </td>
                                            ...
                                            <td className="px-4 py-4 text-sm text-gray-900">{revenue.client}</td>
                                            <td className="px-4 py-4 text-sm text-gray-900">{revenue.service}</td>
                                            <td className="px-4 py-4 text-right font-semibold text-purple-600">€{revenue.amount}</td>
                                            <td className="px-4 py-4 text-sm text-gray-900">{revenue.submittedBy}</td>
                                            <td className="px-4 py-4">
                                                <div className="flex items-center justify-center gap-2">
                                                    <Button variant="primary" size="sm" onClick={() => handleApprove(revenue.id, "revenue")}>
                                                        <Check className="w-4 h-4" />
                                                        Approve
                                                    </Button>
                                                    <Button variant="danger" size="sm" onClick={() => handleReject(revenue.id, "revenue")}>
                                                        <X className="w-4 h-4" />
                                                        Reject
                                                    </Button>
                                                </div>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {approvedItems.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50 transition">
                                            <td className="px-4 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.type === "Revenue" ? "bg-purple-100 text-purple-800" : "bg-pink-100 text-pink-800"
                                                    }`}>
                                                    {item.type}
                                                </span>
                                            </td>
