...
import { CLIENTS as clients, SERVICES as services, WORKERS as workers } from "@/lib/data";

export default function AddIncomePage() {
    const auth = useAuth();
    const { isWorker, isAdmin, user, getWorkerId } = auth;
    ...
    const handleBookingChange = (bookingId: number | null) => {
        ...
        if (booking) {
            ...
            if (mappedServiceIds.length > 0) {
                ...
                const total = mappedServiceIds.reduce((sum, id) => sum + (services.find(s => s.id === id)?.price || 0), 0);
                setBaseAmount(total);
            }
    ...
    const toggleService = (serviceId: number) => {
        setSelectedServices(prev => {
            ...
            const total = newSelection.reduce((sum, id) => sum + (services.find(s => s.id === id)?.price || 0), 0);
            setBaseAmount(total);
            return newSelection;
        });
    };
    ...
    const servicePrestation = baseAmount - discount;
    const subtotal = servicePrestation + tips;
    ...
    const workerShares = assignedWorkers.map(aw => {
        ...
        const serviceShare = (servicePrestation * aw.percentage) / 100;
        const workerServiceAmount = (serviceShare * (worker?.sharingKey || 50)) / 100;
        const salonServiceAmount = serviceShare - workerServiceAmount;
        const tipShare = (tips * aw.percentage) / 100;
        return { ...aw, worker, serviceShare, workerAmount: workerServiceAmount, salonAmount: salonServiceAmount, tipShare };
    });
    const totalWorkerAmount = workerShares.reduce((sum, ws) => sum + ws.workerAmount, 0);
    const totalSalonAmount = workerShares.reduce((sum, ws) => sum + ws.salonAmount, 0) + totalProductsCost;
    ...
    const updateWorkerPercentage = (index: number, newPercentage: number) => {
        setAssignedWorkers(prev => {
            ...
            } else if (index === next.length - 1 && next.length > 1) {
                // If last worker updated, we might need to adjust previous one if we want to keep 100%
                // But usually user expects to be able to set the last one.
                // Request says "le dernier qui est l'ajustement variable", implying it absorbs rounding.
            }
    ...
    const updateWorkerAmount = (index: number, amount: number) => {
        if (servicePrestation <= 0) return;
        const percentage = Number(((amount / servicePrestation) * 100).toFixed(2));
    ...
    const handleSave = async (status: IncomeStatus) => {
        ...
        if (Math.abs(totalPercentage - 100) > 0.01) {
            addToast(`Total worker split must be 100%. Current: ${totalPercentage}%`, "error");
            return;
        }
            ...
            <div className="space-y-6">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="p-4 bg-purple-50 border border-purple-100 rounded-xl space-y-3">
                                ...
                                <select
                                    ...
                                    className="w-full px-4 py-3 border border-purple-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 bg-white text-sm appearance-none cursor-pointer"
                                    style={{
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {services.map(service => (
                                        <button
                                            ...
                                            className={`p-3 rounded-xl border-2 text-left transition-all ${selectedServices.includes(service.id) ? "border-purple-500 bg-purple-50" : "border-gray-200"}`}
                                        >
                                    ...
                                    <button
                                        ...
                                        className={`p-3 rounded-xl border-2 text-left transition-all ${isOtherService ? "border-orange-500 bg-orange-50" : "border-gray-200"}`}
                                    >
                            ...
                            <div className="p-4 bg-gray-50 rounded-xl space-y-4 border border-gray-100">
                                ...
                                {!isNewClient ? (
                                    <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                                        <select
                                            className="w-full px-3 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none bg-white"
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full px-2 py-2 border border-gray-200 rounded-lg text-xs" />
                                            <span className="text-gray-400">-</span>
                                            <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full px-2 py-2 border border-gray-200 rounded-lg text-xs" />
                                        </div>
                                    </div>
                                </div>
                                <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        <div>
                                            ...
                                            <input type="number" value={baseAmount} onChange={(e) => setBaseAmount(Number(e.target.value))} className="w-full p-2 border border-purple-200 rounded-lg text-sm font-bold" />
                                        </div>
                                        <div>
                                            <label className="text-[10px] text-purple-700 mb-1 block">Disc.</label>
                                            <input type="number" value={discount} onChange={(e) => setDiscount(Number(e.target.value))} className="w-full p-2 border border-purple-200 rounded-lg text-sm" />
                                        </div>
                                        <div>
                                            <label className="text-[10px] text-purple-700 mb-1 block">Tips</label>
                                            <input type="number" value={tips} onChange={(e) => setTips(Number(e.target.value))} className="w-full p-2 border border-purple-200 rounded-lg text-sm" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <Card className="border-l-4 border-l-pink-500">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Button variant="success" size="sm" onClick={addWorker}><Plus className="w-4 h-4" /> Add</Button>
                    </div>
                        ...
                        {workerShares.map(({ workerId, percentage, worker, workerAmount, serviceShare, tipShare }, idx) => (
                            <div key={workerId} className={`p-4 rounded-xl bg-gradient-to-br ${worker?.color || "from-gray-500 to-gray-600"} text-white shadow-md relative group transition-all hover:shadow-lg`}>
                                <button
                                    ...
                                    className="absolute top-2 right-2 opacity-30 hover:opacity-100 transition-all hover:scale-110 z-10 text-white"
                                >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="mr-8">
                                        <select
                                            ...
                                            className="w-full bg-white/20 border border-white/30 rounded-lg px-3 h-10 text-white text-sm font-bold focus:outline-none focus:ring-2 focus:ring-white/50 cursor-pointer hover:bg-white/25 transition appearance-none"
                                            style={{
                                            ...
                                            {permissions.canViewSensitiveWorkerFinancials(workerId) ? (
                                                <>
                                                    <input
                                                        ...
                                                        className="w-full bg-white/20 border border-white/30 rounded-lg pl-3 pr-6 h-10 text-white text-sm font-bold focus:outline-none focus:ring-2 focus:ring-white/50 outline-none hover:bg-white/25 transition"
                                                    />
                ...
                <Card className="border-l-4 border-l-teal-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="w-full p-3 border border-gray-300 rounded-xl" placeholder="Service notes..." />
                        </div>
                        <div>
                            <div className="flex justify-between mb-2">
                                <label className="text-sm font-bold text-gray-700">Products</label>
                                <button onClick={addProduct} className="text-xs text-purple-600 font-bold">+ Add</button>
                            </div>
                            <div className="space-y-2">
                                {usedProducts.map((up, idx) => (
                                    <div key={idx} className="flex gap-2 items-center">
                                        <select className="flex-1 p-2 border rounded-lg text-xs" value={up.productId} onChange={(e) => {
                                        ...
                                        <input type="number" className="w-12 p-2 border rounded-lg text-xs" value={up.quantity} onChange={(e) => {
                                            const next = [...usedProducts];
                                        ...
                                        <button onClick={() => removeProduct(up.productId)} className="text-red-400"><Trash2 className="w-3 h-3" /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="mt-6">
                        ...
                        <div className="flex gap-2">
                            {['card', 'cash', 'mobile'].map(m => (
                                <button key={m} onClick={() => setPaymentMethod(m)} className={`px-4 py-2 rounded-lg border-2 text-sm font-bold capitalize transition-all ${paymentMethod === m ? 'border-purple-600 bg-purple-50 text-purple-700' : 'border-gray-200 text-gray-500'}`}>{m}</button>
                            ))}
                        </div>
                    </div>
                </Card>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                            <button
                                ...
                                className="flex-1 py-4 px-6 rounded-2xl border border-purple-300/30 bg-purple-500/20 hover:bg-purple-500/30 text-purple-100 text-sm font-bold tracking-widest transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 group backdrop-blur-sm"
                            >
